...
print(runKoG2P('박물관', 'rulebook.txt'))
...
def get_phon_and_sem_similar_words(query):
    ...
    words = [word for word, score in model.wv.most_similar(query, topn=100)]

    sim_dict = {}

    for word in words:
        word_ipa = runKoG2P(word, 'rulebook.txt')
        B_new = multi.transform([word_ipa.split()])
        f1 = f1_score(A_new, B_new, average='samples')
        sim_dict[word] = f1

    sim_words = [k for k,v in sorted(sim_dict.items(), key=lambda x:x[1], reverse=True) if v > 0.0]
    return sim_words
