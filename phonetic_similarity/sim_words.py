...
ipa_tags = [['p0','ph','pp','t0','th','tt','k0','kh','kk','s0','ss','h0','c0','ch','cc','mm','nn','rr','pf','ph','tf','th','kf','kh','kk',\
's0','ss','h0','c0','ch','mf','nf','ng','ll','ks','nc','nh','lk','lm','lb','ls','lt','lp','lh','ps','ii','ee','qq','aa','xx','vv',\
...
def get_phon_and_sem_similar_words(query):
    ...
    query_ipa = runKoG2P(query, 'rulebook.txt')
    A_new = multi.transform([query_ipa.split()])

    sim_dict = {}

    for word, ipa in zip(words, ipas):
        if not isinstance(ipa, str) or word == query:
            continue
        B_new = multi.transform([ipa.split()])
        f1 = f1_score(A_new, B_new, average='samples') # 발음 유사도 순
        sim = model.wv.similarity(query, word) # 의미 유사도 순
        sim_dict[word] = (f1, sim)

    sim_words = [k for k,v in sorted(sim_dict.items(), key=lambda x:(x[1][0], x[1][1]), reverse=True)][:20]
    return [k for k in sim_words if k in tokenizer.vocab.keys()][:3]

def compute_sim(a,b):

    a_ipa = runKoG2P(a, 'rulebook.txt')
    A_new = multi.transform([a_ipa.split()])

    b_ipa = runKoG2P(b, 'rulebook.txt')
    B_new = multi.transform([b_ipa.split()])

    f1 = f1_score(A_new, B_new, average='samples')

    return f1

def get_ipa_vec(word):
    tags = ['p0', 'ph', 'pp', 't0', 'th', 'tt', 'k0', 'kh', 'kk', 's0', 'ss', 'h0', 'c0', 'ch', 'cc', 'mm', 'nn', 'rr', 'pf',
     'ph', 'tf', 'th', 'kf', 'kh', 'kk', \
     's0', 'ss', 'h0', 'c0', 'ch', 'mf', 'nf', 'ng', 'll', 'ks', 'nc', 'nh', 'lk', 'lm', 'lb', 'ls', 'lt', 'lp', 'lh',
     'ps', 'ii', 'ee', 'qq', 'aa', 'xx', 'vv', \
     'uu', 'oo', 'ye', 'yq', 'ya', 'yv', 'yu', 'yo', 'wi', 'wo', 'wq', 'we', 'wa', 'wv', 'xi']
    answer = {tag:0 for tag in tags}
    ipa_result = runKoG2P(word, 'rulebook.txt')
    for ipa in ipa_result.split():
        answer[ipa] += 1
